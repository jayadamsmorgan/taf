...
static inline void print_delim() { puts("-----------"); }
...
void taf_headless_test_failed(raw_log_test_t *test) {
    ...
    for (size_t i = 0; i < test->failure_reasons_count; i++) {
        ...
        printf("(%s:%d):\n%.*s\n", o->file, o->line, (int)o->msg_len, o->msg);
    }
    print_delim();
}
...
void taf_headless_defer_queue_failed(raw_log_test_output_t *output) {
    printf("%s Defer (%s:%d) failed. Traceback:\n%.*s\n", output->date_time,
           output->file, output->line, (int)output->msg_len, output->msg);
    print_delim();
}
